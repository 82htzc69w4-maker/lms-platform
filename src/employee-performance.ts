  ...
  <div id="performance-content" style="display:none;">
    ...
    <div class="panel">
      <div class="panel-header">
        ...
        <div class="panel-sub">Ask about this employee — grounded only in their real training, assessment, incident, and productivity data above</div>
      </div>
      <div class="panel-body">
        <div id="ai-coach-chat-wrap" style="margin-bottom: 16px;"></div>
        <div class="form-row">
          <input type="text" id="ai-coach-question" placeholder="e.g. Why is this employee's performance dropping?" style="flex:1;" />
...
const scripts = `
  ...
  document.getElementById('employee-select').addEventListener('change', (e) => {
    ...
    document.getElementById('ai-coach-chat-wrap').innerHTML = '';
  });
  ...
  document.getElementById('ai-coach-ask-btn').addEventListener('click', () => {
    ...
    fetch('/api/ai-coach/analyze-employee', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: selectedUsername, question })
    })
      .then(async (r) => {
        const data = await r.json();
