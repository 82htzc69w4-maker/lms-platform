...
const scripts = `
  ...
  function renderBlockHtml(block) {
    ...
    if (block.type === 'heading') {
      ...
      return \`<h2 style="margin:0 0 20px; font-family:\${headingFont}; font-size:\${headingSize}px; text-transform:uppercase; color:var(--text-primary);">\${safeTitle}</h2>\`;
    }

    if (block.type === 'subtitle') {
      const subtitleFont = settings.fontFamily || "'Inter', sans-serif";
      const subtitleSize = settings.fontSize || 15;
      return \`<div style="font-family:\${subtitleFont}; font-size:\${subtitleSize}px; color:var(--text-muted); font-style:italic; margin-bottom:20px;">\${safeTitle}</div>\`;
    }
    ...
    if (block.type === 'text') {
      ...
      return \`<div style="margin-bottom:20px; font-family:\${textFont}; font-size:\${textSize}px; color:var(--text-primary); line-height:1.6;">\${html}</div>\`;
    }
